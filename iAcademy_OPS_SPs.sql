...
--
-- Host: localhost
-- Generation Time: Apr 09, 2014 at 09:10 AM
...
BEGIN
	...
	FROM iops.employers_students_log
	INNER JOIN iops.students 
	ON iops.students.studentID = iops.employers_students_log.studentID
       LEFT JOIN iops.positions
        ON iops.employers_students_log.positionID = iops.positions.positionID 
       LEFT JOIN iops.employers
ON  iops.employers_students_log.employerID = iops.employers.employerID 
...
BEGIN
	SELECT
		...
		, iops.employers_students_log.startDate
		, iops.employers_students_log.endDate
                , iops.employers_students_log.positionID
	FROM iops.employers_students_log
       INNER JOIN iops.students ON iops.students.studentID = iops.employers_students_log.studentID
      LEFT JOIN iops.positions ON iops.employers_students_log.positionID = iops.positions.positionID
       LEFT JOIN iops.employers ON iops.employers_students_log.employerID = iops.employers.employerID
	WHERE iops.employers_students_log.employerID = employerId 
	AND iops.students.isVerified = 1
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `viewAlumnus`(IN `pInternID` INT(11))
    NO SQL
SELECT s.firstName
, s.lastName
, s.middleName
